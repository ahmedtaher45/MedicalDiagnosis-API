...
namespace Diagnosis.Infrastracture.Repositories
{
    public class AdminDashboardRepository : IAdminDashboardRepository
    {
        ...
        public async Task<AdminDashboardDto> GetDashboardDataAsync()
        {
            ...
            var activeDoctors = await _context.Set<Consultation>()
                .Where(c => c.Date >= now.AddDays(-30))
                .Select(c => c.DoctorId)
                .Distinct()
                .CountAsync();
            ...
            var diagnosesOverTime = await _context.Set<Consultation>()
                .Where(c => c.Date >= now.AddMonths(-6))
                .GroupBy(c => new { c.Date.Year, c.Date.Month })
                .Select(g => new DiagnosisOverTimeDto
                {
                    Month = g.Key.Month + "/" + g.Key.Year,
                    DoctorDiagnoses = g.Count(c => c.Type == ConsultationType.Inquiry),
